...
fun SeatAvailabilityScreen(ticketId: String, repo: FirestoreRepository, onBack: () -> Unit) {
    ...
    LaunchedEffect(ticketId) {
        vm.observeTicketAndPassengers(ticketId)
    }

    val totalSeats = ticketDetails?.totalSeats ?: 40
    val occupied = passengers.map { it.seatNumber }.filter { it > 0 }.toSet()
    ...
    Scaffold(
        ...
        Column(
            ...
            Text("Route: ${ticketDetails?.route ?: "-"}", style = MaterialTheme.typography.titleMedium)
            Text("Date: ${ticketDetails?.date ?: "-"}", style = MaterialTheme.typography.bodyMedium)
            Spacer(Modifier.height(12.dp))
